...
def test_fs_1():
    ...
    assert os.path.exists(os.path.join(l.path, "image.toml"))
    assert os.path.exists(os.path.join(l.path, "hello.txt"))


def test_fs_2():
    l = LocalClient(name="test_fs_2")
    open(os.path.join(l.path, "hello.txt"), 'w').write('hello world!')
    new = LocalClient(name="test_fs_2")
    assert os.path.exists(os.path.join(l.path, "image.toml"))
    assert not os.path.exists(os.path.join(l.path, "hello.txt"))
